...
bool Quad2DScene::load(device::GPUDevice &device,
  ...
  // 2D quad: 9 unique vertices + 24 indices (4 quadrants × 2 triangles × 3 verts)
  // Using indexed drawing so the center vertex (shared by all 4 quadrants)
  // correctly receives wave displacement from any quadrant's effects.
  //
  ...
  static constexpr P2 gridPositions[9] = {
      {-0.8f, -0.8f}, // 0: bottom-left
      {0.0f, -0.8f},  // 1: bottom-center
      {0.8f, -0.8f},  // 2: bottom-right
      {-0.8f, 0.0f},  // 3: middle-left
      {0.0f, 0.0f},   // 4: center (shared by all quadrants)
      {0.8f, 0.0f},   // 5: middle-right
      {-0.8f, 0.8f},  // 6: top-left
      {0.0f, 0.8f},   // 7: top-center
      {0.8f, 0.8f},   // 8: top-right
  };
