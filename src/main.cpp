...
static constexpr window::MaterialTag QUAD_2D_MATERIAL_TAG{
    "quad2d_unified_mat", &QUAD_2D_SHADER_TAG};
...
static const window::TextureTag CHECKERBOARD_TEX_TAG{
    "checkerboard_tex", CHECKERBOARD_LAYERS, 1};
static const window::TextureTag ATLAS_TEX_TAG{
    "layer_atlas_tex", ATLAS_LAYERS, 1};
...
public:
  ...
  void unload() override {
    ...
    // Shared textures are not released here - they may be used by other objects
    setLoaded(false);
...
public:
  ...
  bool load(device::GPUDevice &device,
    ...
    // Upload shared texture if not already uploaded (shared with cube scene)
    if (checkerboardTex_ && !checkerboardTex_->isUploaded()) {
      if (!checkerboardTex_->upload(allocator, device)) {
        std::println(stderr, "[{}] Failed to upload checkerboard texture",
                     getName());
...
int main(int argc, char *argv[]) {
  ...
  auto checkerboardTex = std::make_shared<window::Texture>(CHECKERBOARD_TEX_TAG);
