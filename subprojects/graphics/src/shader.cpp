...
bool render::Shader::create_shader_module(device::LogicalDevice *device,
  ...
  for (size_t deviceIdx = 0; deviceIdx < logicalDevices.size(); ++deviceIdx) {
    ...
    for (const auto &stage : stages) {
      ...
      if (!create_shader_module(device, stage.spirvCode, shaderModule)) {
        std::print(
            stderr,
            "Shader - {} - failed to create shader module for stage {}\n",
            identifier, shader_type_to_string(stage.type));
        return false;
      }
...
vk::raii::ShaderModule *render::Shader::get_shader_module(ShaderType type,
...
const std::string &render::Shader::get_identifier() const {
  return identifier;
}
