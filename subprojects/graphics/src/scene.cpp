...
void render::Scene::create_basic_material(MaterialId materialId, bool is2D,
  ...
  // Geometry shaders provide modular pipeline where 3D objects are composed
  // from 2D primitives
...
void render::Scene::create_geometry_material(MaterialId materialId, bool is2D,
                                             bool isTextured) {
...
void render::Scene::create_mesh_material(MaterialId materialId, bool is2D) {
  ...
  // Select shader path
  std::string shaderPath =
      is2D ? "assets/shaders/mesh2d.slang" : "assets/shaders/mesh3d.slang";
