...
render::Window::Window(int width, int height, std::string title)
    : frameBufferResized(false), currentWidth(width), currentHeight(height),
      ...
      atlasQuad3(nullptr), atlasQuad4(nullptr), multiMaterialQuad(nullptr),
...
void render::Window::create_scene_objects() {
  ...
  // Create 3D versions of textured materials for 3D objects like the multi-material cube
...
void render::Window::update_objects_positions() {
  // Calculate scale factors to maintain aspect ratio
  // Objects should maintain their square aspect ratio regardless of window
  // shape
  ...
  if (texturedSquare) {
    ...
    texturedSquare->set_scale(
        glm::vec3(0.15f * objectScaleX, 0.15f * objectScaleY, 1.0f));
  }
  ...
  if (multiMaterialCube) {
    ...
    multiMaterialCube->set_scale(
        glm::vec3(0.2f * objectScaleX, 0.2f * objectScaleY, 0.2f));
  }
}
