...
bool render::LayeredTexture::composite_layers() {
  ...
  // Using dark gray/black background allows bright colored shapes to stand
  // out
  ...
  for (size_t layerIdx = 0; layerIdx < layers.size(); ++layerIdx) {
    ...
    if (!layer.visible || !layer.image) {
      std::print("LayeredTexture - {} - skipping layer {} (visible={}, "
                 "hasImage={})\n",
...
bool render::LayeredTexture::load() {
  ...
  for (size_t i = 0; i < imageFutures.size(); ++i) {
    if (imageFutures[i].valid()) {
      ...
      if (!layers[i].image) {
        std::print(stderr,
                   "LayeredTexture - {} - failed to load layer {} image {}\n",
...
void render::LayeredTexture::set_layer_tint(size_t layerIndex,
                                            const glm::vec4 &tint) {
...
void render::LayeredTexture::set_layer_rotation(size_t layerIndex,
                                                float rotation) {
  if (layerIndex < layers.size()) {
...
void render::LayeredTexture::set_layer_visibility(size_t layerIndex,
                                                  bool visible) {
  if (layerIndex < layers.size()) {
...
std::shared_ptr<render::Image>
render::LayeredTexture::get_composited_image() const {
  return compositedImage;
}

size_t render::LayeredTexture::get_layer_count() const { return layers.size(); }
