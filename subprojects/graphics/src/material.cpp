...
bool Material::create_pipeline(LogicalDevice *device,
  ...
  try {
    ...
    if (!createInfo.descriptorBindings.empty()) {
      ...
      std::vector<vk::DescriptorSetLayout> layouts(maxFrames, *resources.descriptorLayout);
      ...
      resources.descriptorSets = vk::raii::DescriptorSets(device->get_device(), allocInfo);
    }
...
void Material::bind(vk::raii::CommandBuffer &commandBuffer,
  ...
  if (!resources.descriptorSets.empty() && frameIndex < resources.descriptorSets.size()) {
