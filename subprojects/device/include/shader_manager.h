...
public:
   ...
   * @return Compiled shader program, or ShaderError on failure
   */
...
private:
  ...
  bool loadCachedSpirv(const std::string &cachePath,
                       std::vector<uint32_t> &spirvOut);
  bool writeCachedSpirv(const std::string &cachePath,
