...
bool WasmRuntime::initializeEngine() {
    // Create WASM engine
...
void WasmRuntime::shutdownEngine() {
    if (store) {
...
bool WasmRuntime::loadModule(const std::filesystem::path& wasmPath) {
    ...
    if (error) {
        ...
        setError(std::string("Failed to compile WASM module: ") + std::string(message.data, message.size));
    ...
    if (error || trap) {
        if (error) {
            wasm_name_t message;
            wasmtime_error_message(error, &message);
            setError(std::string("Failed to instantiate module: ") + std::string(message.data, message.size));
            wasm_byte_vec_delete(&message);
            wasmtime_error_delete(error);
        }
        if (trap) {
            ...
            setError(std::string("Trap during instantiation: ") + std::string(message.data, message.size));
            wasm_byte_vec_delete(&message);
...
bool WasmRuntime::callFunction(
    ...
    if (!wasmtime_instance_export_get(context, &instance, functionName.c_str(), functionName.size(), &func_extern)) {
    ...
    if (error || trap) {
        if (error) {
            wasm_name_t message;
            wasmtime_error_message(error, &message);
            setError(std::string("Error calling function: ") + std::string(message.data, message.size));
            wasm_byte_vec_delete(&message);
            wasmtime_error_delete(error);
        }
        if (trap) {
            wasm_message_t message;
            wasm_trap_message(trap, &message);
            setError(std::string("Trap during function call: ") + std::string(message.data, message.size));
            wasm_byte_vec_delete(&message);
            wasm_trap_delete(trap);
        }
        return false;
...
void WasmRuntime::setError(const std::string& error) {
    lastError = error;
