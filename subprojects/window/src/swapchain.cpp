...
bool Swapchain::createSecondaryGPUResources() {
  ...
  for (auto *gpu : secondaryGPUDevices_) {
    ...
    try {
      ...
      res.offscreenImage =
          std::make_unique<vk::raii::Image>(gpu->getRaiiDevice(), imageInfo);
      ...
      gpu->getDevice().bindImageMemory(*res.offscreenImage,
                                       *res.offscreenMemory, 0);
      ...
      vk::ImageViewCreateInfo viewInfo{
          ...
          *res.offscreenImage,
          ...
          vk::ImageSubresourceRange{vk::ImageAspectFlagBits::eColor, 0, 1, 0,
                                    1}};
      res.offscreenImageView =
          std::make_unique<vk::raii::ImageView>(gpu->getRaiiDevice(), viewInfo);
      ...
      auto cmdBuffers = gpu->getDevice().allocateCommandBuffers(cmdAllocInfo);
      res.commandBuffer = cmdBuffers[0];

      // Create sync objects
      vk::FenceCreateInfo fenceInfo{vk::FenceCreateFlagBits::eSignaled};
      res.renderFence =
          std::make_unique<vk::raii::Fence>(gpu->getRaiiDevice(), fenceInfo);

      vk::SemaphoreCreateInfo semInfo{};
      res.renderSemaphore =
          std::make_unique<vk::raii::Semaphore>(gpu->getRaiiDevice(), semInfo);
...
bool Swapchain::updateMainDevice(device::GPUDevice *device) {
  if (!device || !device->isInitialized()) {
    std::println(stderr, "[Swapchain] Invalid device to update MainDevice");
    return false;
  }
