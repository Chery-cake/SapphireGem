...
bool Texture::upload(device::VMAAllocator &allocator,
  ...
  for (auto &layer : layers_) {
    ...
    if (!stagingBuffer.isValid()) {
      std::println(stderr,
                   "[Texture] Failed to create staging buffer for: {}",
                   layer.imageTag->name);
    ...
    // Copy pixel data to staging buffer, respecting surface pitch/stride
    {
      ...
      if (!mapped) {
        std::println(stderr,
                     "[Texture] Failed to map staging buffer for: {}",
                     layer.imageTag->name);
        SDL_DestroySurface(rgbaSurface);
        return false;
      }
      ...
      for (uint32_t row = 0; row < height; ++row) {
        const uint8_t *srcRow =
            srcPixels + static_cast<size_t>(srcY + row) * srcPitch +
    ...
    // Staging buffer is automatically freed when it goes out of scope (RAII)
