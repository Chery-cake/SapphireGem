...
template <uint32_t Dim> glm::mat4 Object<Dim>::buildModelMatrix() const {
  ...
  if constexpr (Dim >= 3) {
    ...
    model =
        glm::rotate(model, transform_.rotation[2], glm::vec3(0.0f, 0.0f, 1.0f));
    model =
        glm::rotate(model, transform_.rotation[1], glm::vec3(0.0f, 1.0f, 0.0f));
    model =
        glm::rotate(model, transform_.rotation[0], glm::vec3(1.0f, 0.0f, 0.0f));
  } else if constexpr (Dim == 2) {
    // 2D rotation: first component rotates around Z axis,
    // second component rotates around Z axis as secondary rotation
    model =
        glm::rotate(model, transform_.rotation[0], glm::vec3(0.0f, 0.0f, 1.0f));
