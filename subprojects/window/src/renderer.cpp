...
bool Renderer::createImageSyncObjects() {
  ...
  try {
    for (uint32_t i = 0; i < imageCount; ++i) {
      // renderFinishedSemaphore is per swapchain image: acquiring the same
      // image guarantees its previous present has completed, so the semaphore
      // is safe to reuse. This avoids the validation error caused by
      // signaling a semaphore still in use by the swapchain.
    ...
    std::println(stderr,
                 "[Renderer] Failed to create image sync objects: {}",
                 e.what());
    return false;
  }
}
...
bool Renderer::handleSwapchainRecreation() {
  ...
  // Destroy and recreate image sync objects (count may change with new swapchain)
  destroyImageSyncObjects();
  if (!createImageSyncObjects()) {
    return false;
  }
