...
bool Object<Dim>::initialize(device::VMAAllocator &allocator,
  ...
  // These hold the STATIC base positions (undeformed) — compute shader reads them.
  ...
  // These are written by the compute shader each frame with displaced positions.
  ...
  // No wave-flag computation — effects are handled entirely by the compute shader.
  {
