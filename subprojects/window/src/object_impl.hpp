...
bool Object<Dim>::initialize(device::VMAAllocator &allocator,
  ...
  for (uint32_t i = 0; i < framesInFlight; ++i) {
    auto ubo = allocator.createUniformBuffer(uboSize, std::string(name_) +
                                                           "_ubo_" +
                                                           std::to_string(i));
    if (!ubo.isValid()) {
      std::println(stderr,
...
template <uint32_t Dim>
std::array<float, Dim> Object<Dim>::getScale() const {
  std::lock_guard<std::mutex> lock(objectMutex_);
  return scale_;
}

template <uint32_t Dim>
const Face *Object<Dim>::getFace(uint32_t index) const {
  std::lock_guard<std::mutex> lock(objectMutex_);
  ...
  for (uint32_t i = 0; i < maxRotations; ++i) {
    ...
    if (angle != 0.0f) {
      ...
      if constexpr (Dim >= 2) {
        ...
        // For Y-axis rotation (Dim>=3, i==1), negate to maintain right-hand
        if constexpr (Dim >= 3) {
          if (i == 1) {
            rot[b][a] = s;
            rot[a][b] = -s;
